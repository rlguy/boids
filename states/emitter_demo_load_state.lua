local emitter_demo_load_state = state:new()
...
--      INPUT
--[[----------------------------------------------------------------------]]--
function emitter_demo_load_state.keypressed(key)
end
function emitter_demo_load_state.keyreleased(key)
end
function emitter_demo_load_state.mousepressed(x, y, button)
end
function emitter_demo_load_state.mousereleased(x, y, button)
end
...
--[[----------------------------------------------------------------------]]--
--##########################################################################--
function emitter_demo_load_state.load(str)
...
function emitter_demo_load_state.initialize_audio(level)
end

function emitter_demo_load_state.initialize_camera(level)
  local x = TILE_WIDTH * 65
  local y = TILE_HEIGHT * 30
  local start_pos = vector2:new(x, y)
  local camera = camera2d:new(start_pos)
  level:set_camera(camera)
end

function emitter_demo_load_state.construct_level_map(level)
  -- colours
  ...
  local blend = 0.02
  local g_background = tile_gradient:new(require( dir.."blue"), ncolors)
...
function emitter_demo_load_state.pre_level_load(level)
...
function emitter_demo_load_state.initialize_player(level)
end
function emitter_demo_load_state.initialize_tile_explosions(level)
end
function emitter_demo_load_state.initialize_cube_shard_set(level)
end
function emitter_demo_load_state.initialize_shard_sets(level)
end
function emitter_demo_load_state.initialize_mouse_input(level)
  level:set_mouse(MOUSE_INPUT)
end
function emitter_demo_load_state.initialize_audio_objects(level)
end
function emitter_demo_load_state.initialize_polygonizer(level)
...
function emitter_demo_load_state.post_level_load(level)
...
--[[----------------------------------------------------------------------]]--
--##########################################################################--
function emitter_demo_load_state.update(dt)
...
--[[----------------------------------------------------------------------]]--
--##########################################################################--
function emitter_demo_load_state.draw()
  love.graphics.setBackgroundColor(0, 0, 0, 255)
end

return emitter_demo_load_state
